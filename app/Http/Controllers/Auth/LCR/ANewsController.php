...
namespace App\Http\Controllers\Auth\LCR;
...
class ANewsController extends Controller
{
    public function newPost() {
    	return view('admin.auth.lcr_news.assamese.new_post', ['top_category_id' => 2]);
    }
    ...
    public function allPost() {
        return view('admin.auth.lcr_news.assamese.all_news');
    }
    ...
    public function allPostData(Request $request) {
        ...
        if(empty($request->input('search.value'))) {            
            ...
            $news_data = DB::table('assamese_lcr_news')
                            ->join('assamese_top_category', 'assamese_lcr_news.top_category_id', '=', 'assamese_top_category.id')
                            ->select('assamese_lcr_news.*', 'assamese_top_category.top_category')
                            ->offset($start)
                            ->limit($limit)
                            ->orderBy($order,$dir)
                            ->get();
            ...
            $news_data = DB::table('assamese_lcr_news')
                            ->join('assamese_top_category', 'assamese_lcr_news.top_category_id', '=', 'assamese_top_category.id')
                            ->select('assamese_lcr_news.*', 'assamese_top_category.top_category')
                            ->where('assamese_lcr_news.heading','LIKE',"%{$search}%")
                            ->orWhere('assamese_lcr_news.short_desc', 'LIKE',"%{$search}%")
                            ->orWhere('assamese_lcr_news.long_desc', 'LIKE',"%{$search}%")
                            ->orWhere('assamese_lcr_news.author','LIKE',"%{$search}%")
                            ->orWhere('assamese_lcr_news.time','LIKE',"%{$search}%")
            ...
            $totalFiltered = DB::table('assamese_lcr_news')
                            ->join('assamese_top_category', 'assamese_lcr_news.top_category_id', '=', 'assamese_top_category.id')
                            ->select('assamese_lcr_news.*', 'assamese_top_category.top_category')
                            ->where('assamese_lcr_news.heading','LIKE',"%{$search}%")
                            ->orWhere('assamese_lcr_news.short_desc', 'LIKE',"%{$search}%")
        ...
        if(!empty($news_data)) {
            ...
            foreach ($news_data as $single_data) {
                ...
                if($single_data->status == 1){
                    ...
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_assamese_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_assamese_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_assamese_lcr_news_status', ['newsId' => $single_data->id, 'status' => 0])."\">Un-Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_assamese_lcr_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
                else {
                    $val = "<a class=\"btn btn-success\">Un-Publish</a>";
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_assamese_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_assamese_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_assamese_lcr_news_status', ['newsId' => $single_data->id, 'status' => 1])."\">Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_assamese_lcr_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
    ...
    public function viewNews($newsId) {
        $data  = DB::table('assamese_lcr_news')->where('assamese_lcr_news.id', $newsId)
                    ->join('assamese_top_category', 'assamese_lcr_news.top_category_id', '=', 'assamese_top_category.id')
                    ->select('assamese_lcr_news.*', 'assamese_top_category.top_category')
                    ->get();
    ...
    public function showNewsEditForm($newsId) {
        $news = DB::table('assamese_lcr_news')
                    ->where('assamese_lcr_news.id', $newsId)
                    ->join('assamese_top_category', 'assamese_lcr_news.top_category_id', '=', 'assamese_top_category.id')
                    ->select('assamese_lcr_news.*', 'assamese_top_category.top_category')
                    ->get();

        return view('admin.auth.lcr_news.assamese.edit_post', ['news' => $news]);
    }
