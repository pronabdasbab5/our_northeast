...
class ENewsController extends Controller
{
    public function newPost() {
    	return view('admin.auth.lcr_news.english.new_post', ['top_category_id' => 2]);
    }
    ...
    public function allPost() {
        return view('admin.auth.lcr_news.english.all_news');
    }
    ...
    public function allPostData(Request $request) {
        ...
        if(empty($request->input('search.value'))) {            
            ...
            $news_data = DB::table('english_lcr_news')
                            ->join('english_top_category', 'english_lcr_news.top_category_id', '=', 'english_top_category.id')
                            ->select('english_lcr_news.*', 'english_top_category.top_category')
                            ->offset($start)
                            ->limit($limit)
                            ->orderBy($order,$dir)
                            ->get();
            ...
            $totalFiltered = DB::table('english_lcr_news')
                            ->join('english_top_category', 'english_lcr_news.top_category_id', '=', 'english_top_category.id')
                            ->select('english_lcr_news.*', 'english_top_category.top_category')
                            ->where('english_lcr_news.heading','LIKE',"%{$search}%")
                            ->orWhere('english_lcr_news.short_desc', 'LIKE',"%{$search}%")
        ...
        if(!empty($news_data)) {
            ...
            foreach ($news_data as $single_data) {
                ...
                if($single_data->status == 1){
                    ...
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_english_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_english_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_english_lcr_news_status', ['newsId' => $single_data->id, 'status' => 0])."\">Un-Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_english_lcr_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
                else {
                    $val = "<a class=\"btn btn-success\">Un-Publish</a>";
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_english_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_english_lcr_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_english_lcr_news_status', ['newsId' => $single_data->id, 'status' => 1])."\">Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_english_lcr_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
    ...
    public function viewNews($newsId) {
        $data  = DB::table('english_lcr_news')->where('english_lcr_news.id', $newsId)
                    ->join('english_top_category', 'english_lcr_news.top_category_id', '=', 'english_top_category.id')
                    ->select('english_lcr_news.*', 'english_top_category.top_category')
                    ->get();
    ...
    public function showNewsEditForm($newsId) {
        $news = DB::table('english_lcr_news')
                    ->where('english_lcr_news.id', $newsId)
                    ->join('english_top_category', 'english_lcr_news.top_category_id', '=', 'english_top_category.id')
                    ->select('english_lcr_news.*', 'english_top_category.top_category')
                    ->get();

        return view('admin.auth.lcr_news.english.edit_post', ['news' => $news]);
    }
