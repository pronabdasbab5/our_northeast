...
class ENewsController extends Controller
{
    public function newPost() {
    	return view('admin.auth.positive_news.english.new_post', ['top_category_id' => 1]);
    }
    ...
    public function addPost(Request $request, $top_category_id) {
        ...
        if($request->hasFile('file')) {
            ...
            DB::table('positive_english_news')
            		->insert([
    ...
    public function allPost() {
        return view('admin.auth.positive_news.english.all_news');
    }
    ...
    public function allPostData(Request $request) {
        ...
        $totalData = DB::table('positive_english_news')
        					->count();
        ...
        if(empty($request->input('search.value'))) {            
            ...
            $news_data = DB::table('positive_english_news')
                            ->join('english_top_category', 'positive_english_news.top_category_id', '=', 'english_top_category.id')
                            ->select('positive_english_news.*', 'english_top_category.top_category')
                            ->offset($start)
                            ->limit($limit)
                            ->orderBy($order,$dir)
                            ->get();
            ...
            $news_data = DB::table('positive_english_news')
                            ->join('english_top_category', 'positive_english_news.top_category_id', '=', 'english_top_category.id')
                            ->select('positive_english_news.*', 'english_top_category.top_category')
                            ->where('positive_english_news.heading','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.author','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.time','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.short_desc', 'LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.long_desc', 'LIKE',"%{$search}%")
            ...
            $totalFiltered = DB::table('positive_english_news')
                            ->join('english_top_category', 'positive_english_news.top_category_id', '=', 'english_top_category.id')
                            ->select('positive_english_news.*', 'assamese_top_category.top_category')
                            ->where('positive_english_news.heading','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.author','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.time','LIKE',"%{$search}%")
                            ->orWhere('positive_english_news.short_desc', 'LIKE',"%{$search}%")
        ...
        if(!empty($news_data)) {
            ...
            foreach ($news_data as $single_data) {
                ...
                if($single_data->status == 1){
                    ...
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_english_positive_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_english_positive_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_english_positive_news_status', ['newsId' => $single_data->id, 'status' => 0])."\">Un-Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_english_positive_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
                else {
                    $val = "<a class=\"btn btn-success\">Un-Publish</a>";
                    $action = "<a class=\"btn btn-primary\" href=\"".route('view_english_positive_news', ['newsId' => $single_data->id])."\" target=\"_blank\">view</a><a class=\"btn btn-warning\" href=\"".route('edit_english_positive_news', ['newsId' => $single_data->id])."\" target=\"_blank\">Edit</a><a class=\"btn btn-success\" href=\"".route('change_english_positive_news_status', ['newsId' => $single_data->id, 'status' => 1])."\">Publish</a><a class=\"btn btn-danger\" href=\"".route('delete_english_positive_news', ['newsId' => $single_data->id])."\">Delete</a>";
                }
    ...
    public function viewNews($newsId) {
        $data  = DB::table('positive_english_news')->where('positive_english_news.id', $newsId)
                    ->join('english_top_category', 'positive_english_news.top_category_id', '=', 'english_top_category.id')
                    ->select('positive_english_news.*', 'english_top_category.top_category')
                    ->get();
        ...
        return view('admin.auth.positive_news.english.view_news', ['data' => $data]);
    }
    ...
    public function showNewsEditForm($newsId) {
        $news = DB::table('positive_english_news')
                    ->where('positive_english_news.id', $newsId)
                    ->join('english_top_category', 'positive_english_news.top_category_id', '=', 'english_top_category.id')
                    ->select('positive_english_news.*', 'english_top_category.top_category')
                    ->get();

        return view('admin.auth.positive_news.english.edit_post', ['news' => $news]);
    }
