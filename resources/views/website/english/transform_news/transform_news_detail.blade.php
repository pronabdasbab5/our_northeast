...
<head>
    ...
    <meta property="og:url" content="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="{{ $english_transform_news_single_record[0]->heading }}" />
    <meta property="og:description" content="{{ $english_transform_news_single_record[0]->short_desc }}" />
    <meta property="og:image" content="{{ asset('assets/big_img/'.$english_transform_news_single_record[0]->image.'') }}" />
</head>
...
<div class="positive-news">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <center>
                        <h3 style="background-color: #03c2fc;">Transform</h3>
            ...
            <div class="row">
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                    <div class="single-image">
                        <img src="{{ asset('assets/big_img/'.$english_transform_news_single_record[0]->image.'') }}" alt="Blog single photo">
                    </div>
                    <h3><a href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}">{{ $english_transform_news_single_record[0]->heading }}</a></h3>
                     <span class="author"><a href="#"><i class="fa fa-user-o" aria-hidden="true"></i> {{ $english_transform_news_single_record[0]->author }}</a></span>
                    <span class="date"><i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ Carbon\Carbon::parse($english_transform_news_single_record[0]->created_at)->format('M d Y') }}</span>
                    <span class="like"><a href="#"><i class="fa fa-comment-o" aria-hidden="true"></i>  {{ $english_transform_news_single_record[0]->time }} </a></span>
                    <div style="display: inline-flex;" class="pull-right">
                        <div class="share-btn" style="padding-right: 8px;"><a rel="noopener noreferrer" onclick="ga('send', 'event', { eventCategory: 'whatsappStory', eventAction: 'click', eventLabel: 'sharebutton'});" target="_blank" title="share on whatsapp" class="whatsapp-desktop" href="https://web.whatsapp.com/send?text={{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}" data-text="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->heading) ]) }}" data-href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}"><button class="share-button whatsapp"><i class="fa fa-whatsapp">  Share</i></button></a></div>
                        <div class="share-btn" style="padding-right: 8px;"><iframe id="twitter-widget-1" scrolling="no" frameborder="0" allowtransparency="true" class="twitter-share-button twitter-share-button-rendered twitter-tweet-button" style="position: static; visibility: visible; width: 60px; height: 20px;" title="Twitter Tweet Button" src="https://platform.twitter.com/widgets/tweet_button.0639d67d95b7680840758b6833f06d87.en.html#dnt=false&amp;id=twitter-widget-1&amp;lang=en&amp;original_referer=https%3A%2F%2Fdeveloper.twitter.com%2Fen%2Fdocs%2Ftwitter-for-websites%2Ftweet-button%2Foverview.html&amp;size=m&amp;text=Guides%20%E2%80%94%20Twitter%20Developers&amp;time=1566304093898&amp;type=share&amp;url={{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}"></iframe></div>
                        <div class="share-btn"><a href="https://www.facebook.com/sharer/sharer.php?u={{ route('english_transform_news_detail', ['transform_news_id' => encrypt($english_transform_news_single_record[0]->id) ]) }}&amp;src=sdkpreparse"><img src="{{ asset('web/content/fb.jpg') }}" width="65"></div>
                    </div>
                    <p>
                    	{!! $english_transform_news_single_record[0]->long_desc !!}
                    </p>
                    <div class="admin">
                        <div class="col-md-3">
                ...
                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                    ...
                    <div class="sidebar-area">
                        <div class="recent-post-area hot-news">
                            ...
                            <ul class="news-post">
                            	@if(count($english_transform_latest_three_records) > 0)
                            		@foreach($english_transform_latest_three_records as $key => $item)
		                                <li>
		                                    <div class="row">
		                                            ...
		                                            <div class="item-post">
		                                                <div class="row">
		                                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 paddimg-right-none">
		                                                        <a href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}"><img src="{{ asset('assets/small_img/'.$item->image.'') }}" alt="" title="News image"></a>
		                                                    </div>
		                                                    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
		                                                        <h4><a class="line-clamp-3" href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}"> {{ $item->heading }}</a></h4>
