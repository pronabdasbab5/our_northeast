        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">  
                    <div class="row">
                    	@if(count($latest_tow_english_transform_news) > 0)
                        	@foreach($latest_tow_english_transform_news as $key => $item)
	                        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
	                            <ul class="blog-image-gap">
	                                <li>
	                                    <h3><a class="line-clamp-1" href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}">{{ $item->heading }}</a></h3>
	                                    ...
	                                    <div class="blog-image">
	                                        <a href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}">
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 tab-home">
                    ...
                    <div class="hot-news">
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                ...
                                @if(count($all_english_transform_news) > 0)
                                    @foreach($all_english_transform_news as $key => $item)
                                    <li>
                                        <div class="row">
                                            <div class="col-lg-4 col-md-4 col-sm-12 col-xs-4">
                                                <a href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}"><img src="{{ asset('assets/medium_img_index_page/'.$item->image.'') }}" alt="News image" /></a>
                                            </div>
                                            <div class="col-lg-8 col-md-8 col-sm-12 col-xs-8 content">
                                                <h4><a class="line-clamp-3" href="blog-detail.php">{{ $item->heading }}</a></h4>
                                                <span class="author"><a href="#"><i class="fa fa-user-o" aria-hidden="true"></i> {{ $item->author }}</a></span> 
                                                <span class="date"><i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ Carbon\Carbon::parse($item->created_at)->format('M d Y') }}</span>
                                                <span class="duration" style="padding-left: 10px;"><i class="fa fa-clock-o" aria-hidden="true"></i> {{ $item->time }} </span>
                                                <br><br><a style="padding-top: 20px<br><a style="padding-top: 20px; color: red<br><a style="padding-top: 20px; color: blue;" href="{{ route('english_transform_news_detail', ['transform_news_id' => encrypt($item->id) ]) }}">READ MORE...</a>
                                            </div>
                                        </div>
                                    </li>
                                	@endforeach
                    ...
                    <!-- End what’s hot now -->
                    @if(count($all_english_transform_news) > 0)
				        {{ $all_english_transform_news->links() }}
				    @endif
                </div>
                <!--Sidebar Start Here -->
