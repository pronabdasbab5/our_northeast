...
<head>
    ...
    <meta property="og:url" content="{{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="{{ $assamese_news_single_record[0]->heading }}" />
    <meta property="og:description" content="{{ $assamese_news_single_record[0]->short_desc }}" />
    <meta property="og:image" content="{{ asset('assets/big_img/'.$assamese_news_single_record[0]->image.'') }}" />
</head>
...
<div class="single-blog-page-area">
            ...
            <div class="row">
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                    <div class="single-image">
                        <img src="{{ asset('assets/big_img/'.$assamese_news_single_record[0]->image.'') }}" alt="Blog single photo">
                    </div>
                    <h3><a href="{{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}">{{ $assamese_news_single_record[0]->heading }}</a></h3>
                     <span class="author"><a href="#"><i class="fa fa-user-o" aria-hidden="true"></i> {{ $assamese_news_single_record[0]->author }}</a></span>
                    <span class="date"><i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ Carbon\Carbon::parse($assamese_news_single_record[0]->created_at)->format('M d Y') }}</span>
                    <span class="like"><a href="#"><i class="fa fa-comment-o" aria-hidden="true"></i>  {{ $assamese_news_single_record[0]->time }} </a></span>
                    <div style="display: inline-flex;" class="pull-right">
                        <div class="share-btn" style="padding-right: 8px;"><a rel="noopener noreferrer" onclick="ga('send', 'event', { eventCategory: 'whatsappStory', eventAction: 'click', eventLabel: 'sharebutton'});" target="_blank" title="share on whatsapp" class="whatsapp-desktop" href="https://web.whatsapp.com/send?text={{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}" data-text="{{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->heading) ]) }}" data-href="{{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}"><button class="share-button whatsapp"><i class="fa fa-whatsapp">  Share</i></button></a></div>
                        <div class="share-btn" style="padding-right: 8px;"><iframe id="twitter-widget-1" scrolling="no" frameborder="0" allowtransparency="true" class="twitter-share-button twitter-share-button-rendered twitter-tweet-button" style="position: static; visibility: visible; width: 60px; height: 20px;" title="Twitter Tweet Button" src="https://platform.twitter.com/widgets/tweet_button.0639d67d95b7680840758b6833f06d87.en.html#dnt=false&amp;id=twitter-widget-1&amp;lang=en&amp;original_referer=https%3A%2F%2Fdeveloper.twitter.com%2Fen%2Fdocs%2Ftwitter-for-websites%2Ftweet-button%2Foverview.html&amp;size=m&amp;text=Guides%20%E2%80%94%20Twitter%20Developers&amp;time=1566304093898&amp;type=share&amp;url={{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}"></iframe></div>
                        <div class="share-btn"><a href="https://www.facebook.com/sharer/sharer.php?u={{ route('news_detail', ['news_id' => encrypt($assamese_news_single_record[0]->id) ]) }}&amp;src=sdkpreparse"><img src="{{ asset('web/content/fb.jpg') }}" width="65"></div>
                    </div>
                    <p>
                    	{!! $assamese_news_single_record[0]->long_desc !!}
                    </p>
                    <div class="admin">
                        <div class="col-md-3">
                ...
                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                    ...
                    <div class="sidebar-area">
                        <div class="recent-post-area hot-news">
                            ...
                            <ul class="news-post">
                            	@if(count($latest_three_assamese_positive_news) > 0)
                            		@foreach($latest_three_assamese_positive_news as $key => $item)
		                                <li>
		                                    <div class="row">
    ...
    <div class="like-section container">
        ...
        <div class="row" style="color: #101010;">
        	@if(count($assamese_latest_four_records))
        		@foreach($assamese_latest_four_records as $key => $item)
	            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
	                <div class="popular-post-img">
	                    <a href="{{ route('positive_news_detail', ['news_id' => encrypt($item->id) ]) }}"><img src="{{ asset('assets/medium_img/'.$item->image.'') }}" alt="Blog single photo"></a>                                   
	                </div>                                
	                <h3>
	                    <a class="line-clamp-2" href="{{ route('positive_news_detail', ['news_id' => encrypt($item->id) ]) }}">{{ $item->heading }}</a>
	                </h3>
	                <span class="date"><i class="fa fa-calendar-check-o" aria-hidden="true"></i> {{ Carbon\Carbon::parse($item->created_at)->format('M d Y') }}</span>
