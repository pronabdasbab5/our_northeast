        ...
        <div class="container">
            <div class="row">   
                ...
                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 paddimg-left-none paddimg-right-none trending-news">
                    <div class="view-area">
                        <h3 class="">শহতীয়া খবৰ</h3>
                    </div>
                    @if(count($assamese_latest_tow_records) > 0)
                        @foreach($assamese_latest_tow_records as $key => $item)
                            <div class="list-col">
                                <a href="{{ route('news_detail', ['news_id' => encrypt($item->id) ]) }}"> <img src="{{ asset('assets/medium_img/'.$item->image.'') }}" alt="" title="Trending image" width="370" height="230"></a>
                                <div class="dsc">
                                    <p><a class="line-clamp-1" href="{{ route('news_detail', ['news_id' => encrypt($item->id) ]) }}">{{ $item->heading }} </a></p>
                                </div>
                            </div>
                        @endforeach
                    @endif
                        ...
                        <div class="">
                            <div class="row overflowing-row">
                                <div class="col-sm-12 col-xs-12">
                                    <div class="view-area">
                                        <h3 class="">ইতিবাচক খবৰ</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-inner">
                            @if(count($latest_assamese_positive_news) > 0)
                                @php
                                    $slide_active_cnt = 1;
                                @endphp
                                <!-- Item -->
                                @foreach($latest_assamese_positive_news as $key => $item)
                                    <div class="item {{ ($slide_active_cnt == 1)? 'active': 'next' }} left">
                                        <a href="{{ route('positive_news_detail', ['news_id' => encrypt($item->id) ]) }}"><img src="{{ asset('assets/positive_news_index_page/'.$item->image.'') }}" alt="" title="#slider-direction-1"></a>
                                        <div class="dsc">
                                            <span class="date">
                                            ...
                                            <h4><a class="line-clamp-2" href="{{ route('positive_news_detail', ['news_id' => encrypt($item->id) ]) }}"> {{ $item->heading }}</a></h4>
                ...
                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 paddimg-left-none paddimg-right-none trending-news">
                    ...
                    @if(count($assamese_second_latest_tow_records) > 0)
                        @foreach($assamese_second_latest_tow_records as $key => $item)                    
                            <div class="list-col">
                                <a href="{{ route('news_detail', ['news_id' => encrypt($item->id) ]) }}"> <img src="{{ asset('assets/medium_img/'.$item->image.'') }}" alt="" title="Trending image" width="370" height="230"></a>
                                <div class="dsc">
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                ...
                                <ul class="news-post news-feature-mb">
                                    @if(count($assamese_latest_four_records) > 0)
                                        @foreach($assamese_latest_four_records as $key => $item)
                                            <li>
                                                <div class="row">
                                                    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-4">
                                                        <a href="{{ route('news_detail', ['news_id' => encrypt($item->id) ]) }}"><img src="{{ asset('assets/medium_img/'.$item->image.'') }}" alt="News image" /></a>
                                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12 text-center">
                                        <a href="{{ route('all_news_list') }}"><i class="fa fa-angle-double-left" aria-hidden="true"></i>  View More</a>
                                    </div>
                                </div>
                            </div>
                        </div>
